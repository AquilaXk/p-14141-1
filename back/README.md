...
12. `ActorApplicationService`와 `MemberProxy`: 토큰에서 도메인 회원으로
...
22. `MemberApplicationService`: 회원 유스케이스 오케스트레이터
23. Member API 컨트롤러
...
34. ApplicationService 패턴: Controller가 직접 하지 않는 이유
...
- 컨트롤러는 얇고, `ApplicationService` 가 유스케이스를 수행하며, 엔티티는 정책과 상태 변경 책임을 일부 가진다.
...
flowchart TD
    ...
    Controller --> ApplicationService[ApplicationService / Use Case]
    ApplicationService --> Domain[Domain Entity]
    ApplicationService --> Repo[Repository]
    ApplicationService --> Event[Event Publisher]
    Repo --> DB[(PostgreSQL)]
    Security --> Actor[Rq / ActorApplicationService]
...
- 진짜 업무 흐름은 `ApplicationService` 가 잡는다.
...
- `adapter/web|bootstrap|scheduler`: 외부 입력 채널
- `application/service`: 유스케이스 오케스트레이션
- `application/port/input|output`: 입력/출력 계약
- `domain`: 핵심 비즈니스 상태와 규칙
- `adapter/persistence|storage|security|external`: DB/외부 연동
...
class ApiV1PostController(
    ...
    fun write(@Valid @RequestBody reqBody: PostWriteRequest): RsData<PostDto> {
        val post = postApplicationService.write(
            rq.actor,
            reqBody.title,
            reqBody.content,
...
3. 실제 쓰기 작업은 `postApplicationService.write(...)` 에 위임한다.
...
실제 로직은 대부분 `ApplicationService`, `Domain`, `RepositoryImpl` 에 있다.

---

## 7. 두 번째 핵심 관찰: ApplicationService 가 유스케이스를 수행한다

이번에는 `PostApplicationService.write` 를 보자.
...
즉 `ApplicationService` 는 그냥 서비스 계층이 아니라, 이 프로젝트에서는 유스케이스 오케스트레이터에 더 가깝다.
...
sequenceDiagram
    ...
    participant A as ActorApplicationService
...
private fun authenticateIfPossible(request: HttpServletRequest, response: HttpServletResponse) {
    ...
    val member = actorApplicationService.findByApiKey(apiKey)
...
fun login(
    ...
    val member = memberApplicationService.findByUsername(reqBody.username)
        ?: throw AppException("401-1", "존재하지 않는 아이디입니다.")

    memberApplicationService.checkPassword(member, reqBody.password)
...
sequenceDiagram
    ...
    participant PF as PostApplicationService
...
- 유스케이스는 `ApplicationService` 가 조립한다.
- 엔티티는 상태와 규칙을 가진다.
...
6. `back/src/main/kotlin/com/back/global/web/application/Rq.kt`
7. `back/src/main/kotlin/com/back/boundedContexts/member/adapter/web/ApiV1AuthController.kt`
8. `back/src/main/kotlin/com/back/boundedContexts/post/adapter/web/ApiV1PostController.kt`
9. `back/src/main/kotlin/com/back/boundedContexts/post/application/service/PostApplicationService.kt`
10. `back/src/main/kotlin/com/back/boundedContexts/post/domain/Post.kt`
11. `back/src/main/kotlin/com/back/boundedContexts/post/adapter/persistence/PostRepositoryImpl.kt`
12. 그다음에 테스트 코드
...
5. 컨트롤러와 `ApplicationService` 의 책임 차이는 무엇인가?
6. `Rq` 는 왜 필요한가?
...
├── adapter/      ← web/bootstrap/persistence
├── config/       ← 이 문맥 전용 설정
├── domain/       ← 엔티티, 도메인 정책
├── dto/          ← 데이터 전송 객체
...
- `adapter/web|event|scheduler`: 바깥 세상이 들어오는 입구
- `application/service`: 유스케이스 실행
- `application/port/output` + `adapter/persistence|external|storage`: 바깥으로 나가는 출구

즉 데이터 흐름은 `외부 → adapter(web) → application(service) → domain → adapter(persistence/external) → DB/외부시스템` 방향이다.
...
global/
├── app/       ← AppConfig
├── application/ ← AppFacade, EventPublisher, AppException, Rq, Task/Storage 서비스
...
4강에서는 HTTP 요청 1개가 이 구조를 어떻게 통과하는지를 따라간다. 필터, 컨트롤러, ApplicationService, 레포지토리까지 한 흐름으로 연결해서 보면 3강의 폴더 구조가 완성된다.
...
2. 현재 로그인 사용자(`rq.actor`)와 함께 `ApplicationService` 에 위임한다.
...
## 3. ApplicationService: 유스케이스를 조립한다
...
게시글 저장 이후 일어나야 하는 부수효과(로그 기록, 알림 발송 등)는 이벤트로 분리한다. ApplicationService가 직접 그 일을 하지 않는다. 이벤트 리스너가 받아서 처리한다.
...
| `ApplicationService` | 유스케이스: 무슨 일을 할 것인가 |
...
val payloadMember = accessToken
    .takeIf { it.isNotBlank() }
    ?.let(actorApplicationService::payload)          // AuthTokenService.payload() 호출
    ?.let { Member(it.id, it.username, null, it.name) }
```

`actorApplicationService.payload(accessToken)` 가 `AuthTokenService.payload()` 를 호출한다. 결과가 null 이면 토큰이 만료됐거나 위조된 것이다. 결과가 있으면 DB 조회 없이 `Member` 객체를 만들어 인증한다.
...
12강에서는 `ActorApplicationService` 와 `MemberProxy` 를 본다. 필터에서 인증된 뒤, 컨트롤러에서 `rq.actor` 를 꺼낼 때까지 어떤 경로를 거치는지를 다룬다.

---

# 12강. `ActorApplicationService`와 `MemberProxy`: 토큰에서 도메인 회원으로
...
## 3. `ActorApplicationService.memberOf()`: SecurityUser → Member
...
@Component
@RequestScope
class Rq(private val actorApplicationService: ActorApplicationService) {
    val actorOrNull: Member?
        get() = (SecurityContextHolder.getContext()?.authentication?.principal as? SecurityUser)
            ?.let { actorApplicationService.memberOf(it) }
...
```kotlin
val post = postApplicationService.write(rq.actor, ...)
```
...
`ApiV1AuthController` 는 `back/src/main/kotlin/com/back/boundedContexts/member/adapter/web/` 에 있다.
...
sequenceDiagram
    ...
    participant AF as ActorApplicationService
    ...
    AF-->>RQ: MemberProxy
    RQ-->>CT: Member
    CT->>CT: postApplicationService.write(rq.actor, ...)
```
...
val member = memberApplicationService.modifyOrJoin(username, password, nickname, profileImgUrl).data
...
```kotlin
val post = postApplicationService.findById(id).getOrThrow()
```
...
└── subContexts/
    └── memberActionLog/
        ├── application/service/MemberActionLogApplicationService.kt
        ├── application/port/output/MemberActionLogRepositoryPort.kt
        ├── domain/MemberActionLog.kt
        ├── dto/MemberCreateActionLogPayload.kt
        ├── adapter/event/MemberActionLogEventListener.kt
        └── adapter/persistence/MemberActionLogRepository.kt
```
...
`member` 문맥 아래 `domain/shared/` 같은 경로는 컨텍스트 간 공유가 필요한 타입(예: `Member`)을 둔다.

예를 들어 `post` 문맥이 `Member` 엔티티를 참조할 때 `member/domain/shared/Member.kt` 를 가져온다. 반면 유스케이스 오케스트레이션은 `application/service` 계층에서 담당한다.
...
| 폴더 | 역할 |
|---|---|
| `adapter/web`, `adapter/bootstrap`, `adapter/event`, `adapter/scheduler` | 외부 입력 채널별 어댑터 |
| `application/service` | 유스케이스를 실행하는 오케스트레이터 |
| `application/port/input`, `application/port/output` | 입력 유스케이스 계약, 출력 포트 계약 |
| `domain/` | 엔티티, 정책, mixin |
| `adapter/persistence`, `adapter/external`, `adapter/storage`, `adapter/mail` | DB/외부 연동 구현 |
...
## 4. `application/service/` 의 의미

`ActorApplicationService` 와 `AuthTokenService` 가 `application/service/` 에 있다. 이것들은 인증 필터(`CustomAuthenticationFilter`) 와 OAuth2 성공 핸들러에서 호출된다.

`MemberApplicationService` 도 `application/service/` 아래 있고, 회원 유스케이스 오케스트레이션을 담당한다.
...
상태를 바꾸는 메서드가 엔티티 안에 있다. ApplicationService나 컨트롤러가 `member.nickname = "..."` 로 직접 필드를 건드리지 않는다.
...
21강에서는 `MemberAttr` 와 `MemberPolicy` 를 간단히 정리하고, 22강에서 `MemberApplicationService` 로 넘어간다.
...
22강에서는 `MemberApplicationService` 를 본다. 회원 가입, 로그인 검증, 수정, `modifyOrJoin` 을 한 눈에 이해한다.

---

# 22강. `MemberApplicationService`: 회원 유스케이스 오케스트레이터

22강 목표는 이것이다.

> 회원과 관련된 주요 동작이 `MemberApplicationService` 에서 어떻게 처리되는지 파악하기.
...
반환이 `RsData<Member>` 인 점을 눈여겨보자. ApplicationService가 `RsData` 를 반환하는 드문 경우다. 가입인지 수정인지를 호출자(OAuth2 서비스)가 알아야 하기 때문이다.
...
`Boolean` 을 반환하지 않고 실패 시 예외를 던진다. 컨트롤러에서 `if (!memberApplicationService.checkPassword(...)) return error` 같은 코드가 없어도 된다.
...
fun join(@Valid @RequestBody reqBody: MemberJoinRequest): RsData<MemberDto> {
    val member = memberApplicationService.join(reqBody.username, reqBody.password, reqBody.nickname, null)
...
`adapter/bootstrap/` 폴더에 `MemberNotProdInitData.kt` 가 있다. 이건 개발 환경에서 초기 데이터를 만드는 컴포넌트다. `@Profile("!prod")` 로 운영 환경에서는 실행되지 않는다. 컨트롤러와 역할이 다르기 때문에 `web` 과 분리해 `bootstrap` 으로 둔다.
...
잠깐 — 실제로 `Member` 가 직접 `PostMember` 를 구현하지 않으면 `author.incrementPostsCount()` 가 어떻게 컴파일되는가? `PostApplicationService.write` 코드를 보면 `author.incrementPostsCount()` 를 호출한다. `Member` 가 `PostMember` 를 구현해야만 이 호출이 가능하다. 실제 코드에서 `Member` 는 `PostMember` 를 구현하고 있다.
...
# 33강 — PostApplicationService — 유스케이스 오케스트레이터

`PostApplicationService` 는 `post` 컨텍스트의 유일한 서비스 빈이다. 모든 글·댓글·좋아요 관련 유스케이스가 여기서 처리된다.
...
컨트롤러는 얇다. 검증과 권한 확인 후 ApplicationService 에 위임하고, 응답 DTO 를 만들어 반환하는 역할만 한다.
...
private fun makePostDtoPage(postPage: Page<Post>): PageDto<PostDto> {
    ...
    val likedPostIds = postApplicationService.findLikedPostIds(actor, postPage.content)
...
fun modify(@PathVariable @Positive id: Int, @Valid @RequestBody reqBody: PostModifyRequest): RsData<PostDto> {
    ...
    postApplicationService.modify(rq.actor, post, reqBody.title, reqBody.content, reqBody.published, reqBody.listed)
...
fun getOrCreateTemp(response: HttpServletResponse): RsData<PostWithContentDto> {
    val (post, isNew) = postApplicationService.getOrCreateTemp(rq.actor)
    return if (isNew) {
        response.status = 201
...
`PostApplicationService` 에서 글을 저장하면 이벤트를 발행한다. 이 강의는 그 이벤트 시스템이 어떻게 구성되는지를 설명한다.
...
test/
  member/
    adapter/bootstrap/MemberNotProdInitDataTest.kt
    adapter/web/ApiV1AuthControllerTest.kt        ← MockMvc 기반 API 테스트
    adapter/web/ApiV1MemberControllerTest.kt
    adapter/web/ApiV1AdmMemberControllerTest.kt
    adapter/persistence/MemberRepositoryTest.kt
    application/service/AuthTokenServiceTest.kt         ← 단위에 가까운 통합 테스트
    application/service/ActorApplicationServiceTest.kt
    application/service/MemberApplicationServiceTest.kt
    domain/shared/MemberTest.kt
    domain/shared/memberExtensions/MemberSecurityExtensionsTest.kt
  post/
    adapter/bootstrap/PostNotProdInitDataTest.kt
    adapter/web/ApiV1PostControllerTest.kt
    adapter/web/ApiV1PostCommentControllerTest.kt
    adapter/web/ApiV1AdmPostControllerTest.kt
```
...
class ApiV1AuthControllerTest {
    ...
    @Autowired private lateinit var memberApplicationService: MemberApplicationService
...
postApplicationService.delete(post, rq.actor)
```
...
같은 사람이라도 역할이 다르다. `PostApplicationService.writeComment` 를 보면:
...
`PostApplicationService` 는 `post.modify(...)` 를 호출하기만 한다. 규칙은 `Post` 가 알고 있다.
...
```
adapter/web|event|scheduler  ← 컨트롤러, 이벤트 리스너, 스케줄러
adapter/persistence|external|storage  ← Repository, 외부 API 클라이언트
application/service  ← 유스케이스 오케스트레이션
application/port/input|output  ← 입력/출력 계약
domain/ ← 엔티티, 도메인 로직
```

헥사고날 아키텍처의 포트-어댑터 패턴을 따르되, Kotlin 예약어(`in`)를 피하기 위해 역할 기반 이름을 사용한다.
...
프론트엔드는 **Next.js (Pages Router)** + **React** + **TypeScript** + **TailwindCSS** + **shadcn/ui** 스택이다.
...
front/src/
  pages/            ← Next.js Pages Router 엔트리
    ...
    admin/tools.tsx
  routes/           ← 페이지 본문 조합
  apis/backend/     ← 백엔드 API 계약
  hooks/            ← 상태/데이터 훅
  layouts/          ← 공통 레이아웃
  components/ui/    ← shadcn/ui 컴포넌트
  libs/             ← 공유 유틸/비즈니스 컴포넌트
  styles/           ← 스타일 토큰
```
...
## 3. SSR(getServerSideProps)에서 쿠키 전달

```typescript
// src/pages/posts/[id].tsx (SSR 엔트리)
export const getServerSideProps = async (ctx) => {
  ...
  const res = await client.GET("/post/api/v1/posts/{id}", {
    params: { path: { id } },
    headers: { cookie }, // 서버 측 쿠키 명시적 전달
  });

  return { props: { initialPost: res.data ?? null } };
};
```

Pages Router SSR에서도 브라우저 쿠키를 자동 전달하지 않으므로 `ctx.req.headers.cookie` 를 백엔드 호출 헤더에 수동으로 넘긴다.

이렇게 하면 서버에서 인증된 응답을 받아 초기 HTML에 포함할 수 있다.

---

# 49강 — 글 상세 페이지 — SSR 엔트리와 클라이언트 분리

`/posts/[id]` 페이지는 SSR 엔트리(`pages/posts/[id].tsx`)와 클라이언트 렌더링 로직을 분리해 운영한다.

---

## 1. pages/posts/[id].tsx (SSR 엔트리)
...
[ApiV1PostController.write()]
  @Valid reqBody 검증
  val post = postApplicationService.write(rq.actor, title, content, ...)

[PostApplicationService.write()]  @Transactional
...
fun delete(@PathVariable postId: Int, @PathVariable id: Int): RsData<Void> {
    val post = postApplicationService.findById(postId).getOrThrow()
    val postComment = post.findCommentById(id).getOrThrow()
    postComment.checkActorCanDelete(rq.actor)
    postApplicationService.deleteComment(post, postComment, rq.actor)
...
4. `postApplicationService.deleteComment(post, postComment, rq.actor)` 위임
...
# 55강 — 글 작성/수정 페이지 — zod + react-hook-form + Cmd+S

`/admin/posts/new` 와 관리자 글 편집 화면은 이 프로젝트에서 가장 복잡한 프론트엔드 영역이다.
...
/admin/posts/new
  ↓
제목/내용 입력 후 Cmd+S 또는 저장 버튼
  ↓
POST 또는 PUT /post/api/v1/posts...
  ↓
...
**프론트엔드 (Next.js Pages Router, React, TypeScript)**
- Pages Router SSR과 클라이언트 컴포넌트 분리
...
@Profile("!prod")
@Configuration
class MemberNotProdInitData(private val memberApplicationService: MemberApplicationService) {
    @Lazy
    @Autowired
    ...
    @Transactional
    fun makeBaseMembers() {
        if (memberApplicationService.count() > 0) return

        val memberSystem = memberApplicationService.join("system", "1234", "시스템", null)
        memberSystem.modifyApiKey(memberSystem.username)  // apiKey = "system"

        memberApplicationService.join("admin", "1234", "관리자", null).also { it.modifyApiKey(it.username) }
        memberApplicationService.join("user1", "1234", "유저1", null).also { it.modifyApiKey(it.username) }
        memberApplicationService.join("user2", "1234", "유저2", null).also { it.modifyApiKey(it.username) }
        memberApplicationService.join("user3", "1234", "유저3", null).also { it.modifyApiKey(it.username) }
    }
}
```
...
**멱등성**: `memberApplicationService.count() > 0` 이면 아무것도 하지 않는다. 재시작해도 데이터가 중복 생성되지 않는다.
...
@Transactional
fun makeBasePosts() {
    if (postApplicationService.count() > 0) return

    val memberUser1 = memberApplicationService.findByUsername("user1").getOrThrow()
    // ...
    postApplicationService.write(memberUser1, "제목 1", "내용 1", true, true)
    postApplicationService.write(memberUser1, "비공개 글", "비공개 내용", false, false)
}
```
...
Spring 이 `"CREATED_AT"` 문자열을 `PostSearchSortType1.CREATED_AT` 으로 자동 변환한다. ApplicationService 에서:

```kotlin
PageRequest.of(page - 1, pageSize, sort.sortBy)
```

`sort.sortBy` 를 바로 쓴다. 컨트롤러 → ApplicationService → Repository 전 계층에서 타입이 보장된다. `"createdAt_desc"` 같은 문자열을 파싱하거나 `if/else` 로 분기할 필요가 없다.
...
```kotlin
@Service
class CustomUserDetailsService(private val actorApplicationService: ActorApplicationService) : UserDetailsService {
    override fun loadUserByUsername(username: String): UserDetails {
        val member = actorApplicationService.findByUsername(username)
...
**도메인 중심 설계**
비즈니스 규칙을 엔티티와 Mixin 에 넣는다. ApplicationService 는 얇다. 테스트에서 ApplicationService 를 직접 호출해서 비즈니스 로직을 검증할 수 있다.
...
**완전한 헥사고날 아키텍처**
`adapter/*`, `application/port/input|output` 로 경계를 분리했지만, 문맥 간 의존과 운영 편의성 때문에 완전 분리형 아키텍처까지는 가지 않았다.
