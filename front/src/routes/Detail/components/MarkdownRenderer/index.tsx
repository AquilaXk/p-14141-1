...
const hasMermaidConnectorOrKeyword = (source: string) => {
  ...
  // CodeQL(js/bad-tag-filter) 오탐/우회 여지를 줄이기 위해
  // HTML comment close 리터럴(--> 등) 정규식 매칭 대신 문자 코드 스캔으로 판별한다.
...
const extractPlainTextByHtmlScanner = (rawHtml: string) => {
  ...
  while (index < rawHtml.length) {
    ...
    while (cursor < rawHtml.length && isAsciiWhitespace(rawHtml[cursor])) {
      cursor += 1
    }

    const tagNameStart = cursor
    while (cursor < rawHtml.length && isAsciiAlphaNumeric(rawHtml[cursor])) {
      cursor += 1
    }
    const tagName = rawHtml.slice(tagNameStart, cursor).toLowerCase()

    while (cursor < rawHtml.length && rawHtml[cursor] !== ">") {
      cursor += 1
    }

    if (tagName === "br" || (isClosingTag && BLOCK_BREAK_TAGS.has(tagName))) {
      plainText += "\n"
    }
