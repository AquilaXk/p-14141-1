...
- Mermaid 다이어그램은 원본 SVG 폭을 그대로 쓰지 않는다. 렌더 후 원본 폭을 읽어서 `작은 다이어그램은 적당히 키우고`, `큰 다이어그램은 상한 폭 안에 가두고`, 본문 카드 안에서만 스크롤되도록 stage 폭을 정규화한다.
...
- Mermaid는 기본 스타일을 그대로 두지 말고, 프로젝트 타이포그래피와 패널 톤에 맞춘 가독성 중심 테마를 적용한다.
- Mermaid가 조잡하게 보이면 본문 CSS보다 렌더 훅의 `themeVariables`, `edgeLabelBackground`, node/cluster spacing을 먼저 조정한다.
