...
- 본문 후처리 훅은 실제 마크업이 있을 때만 로드한다. 특히 `mermaid`는 `language-mermaid` 블록이 존재할 때만 동적 import 한다.
- 콜아웃은 raw HTML hydration에 기대지 말고, 마크다운 세그먼트 파싱 단계에서 `<aside>`를 구조화된 callout segment로 변환하는 쪽을 우선한다.
...
- 언어별 Prism 로더는 토큰이 비어 보이지 않도록 선행 의존 언어(`clike` 등)를 함께 로드한다.
- Mermaid는 시각 품질이 중요하지만 기본 렌더를 그대로 쓰면 읽기 어려워질 수 있으므로, 프로젝트 테마에 맞춘 `themeVariables`와 `themeCSS`를 함께 관리한다.
- 코드블럭 툴바(언어 배지)와 줄번호 gutter, 하단 우측 복사 액션은 렌더 비용이 작고 사용성 이득이 크므로, 상세 페이지 route-specific 비용 안에서 제공하는 편이 낫다.
