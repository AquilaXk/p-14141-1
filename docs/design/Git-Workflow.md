...
- 변경 검증과 배포 브랜치 규칙을 빠르게 확인할 때 이 문서를 먼저 읽는다.
- 이 저장소에서 `main`은 실제 운영 배포 브랜치이므로, 로컬 검증과 커밋 범위 관리가 중요하다.
- CI/CD 상세 단계는 `DevOps.md`, 운영 런타임은 `Infrastructure-Architecture.md`로 이어서 보면 된다.
